...
- When developing or modifying items, blocks, entities, or core mechanics in this project, you must follow this automated test-driven workflow:
- Implement Feature**: Write or modify the required Java code and DataGen classes.
- Write GameTest: Create or update corresponding `@GameTest` methods under `src/gametest/java/` (or the project's dedicated test directory).
- Run Automated Server Test: Execute the GameTest task:
   ```bash
...
## Git Commit Rules
- No Co-authors: When committing via Git, Never include or append co-author information (such as `Co-authored-by: ...`) in the commit message or footer.

## Automation Pipeline
- Once the build succeeds completely, automatically run the Git workflow (ensuring no `Co-authored-by` headers are appended):
  ```bash
  git add .
